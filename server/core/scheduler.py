...
class PollingScheduler:
    ...
    async def start(self, poll: bool = False):
        self._running = True
        logger.info("Server Scheduler: Starting Metrics Pruner only (active polling is disabled)")
    ...
    def force_poll(self, device_id: int):
        """No-op on server instances where active polling is disabled."""
        logger.info(f"force_poll ignored for device #{device_id}: polling disabled on server")
