...
async def test_oid(body: dict):
    """Fetch a live OID value from a device. Disabled on server."""
    raise HTTPException(
        status_code=400,
        detail="Active SNMP operations are disabled on server instances."
    )


@router.post("/walk-oid")
async def walk_oid(body: dict):
    """Walk an OID subtree on a device — returns all child OIDs and values. Disabled on server."""
    raise HTTPException(
        status_code=400,
        detail="Active SNMP operations are disabled on server instances."
    )
