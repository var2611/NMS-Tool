...
async def force_poll(
    ...
    """Immediately poll a device. Disabled on server instances."""
    raise HTTPException(
        status_code=400,
        detail="Active polling is disabled on server instances. Devices are polled by their local desktop agents."
    )
...
async def get_device_interfaces(
    ...
    if settings.is_server:
        raise HTTPException(
            status_code=400,
            detail="Active interface monitoring configuration is restricted to local desktop agents."
        )
    result = await db.execute(select(Device).where(Device.id == device_id))
    device = result.scalar_one_or_none()
    if not device:
