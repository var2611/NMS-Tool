      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={submit} className="p-5 space-y-4">
          ...
          <div className="rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
            <div className="bg-gray-50 dark:bg-gray-800 px-4 py-2 text-xs font-semibold text-gray-500 uppercase tracking-wider">
              SNMP Settings
            </div>
            <div className="p-4 grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input className="input font-mono" placeholder="public"
                  ...
                  disabled={form.snmp_version === 'v3'} />
                {form.snmp_version === 'v3' && (
                  <p className="text-xs text-amber-600 mt-1">SNMPv3 uses username/auth — configure after adding</p>
                )}
              </div>
              <div>
                <label className="label">
                  SNMP Port
                  <span className="text-xs text-gray-400 font-normal ml-1">(default: 161)</span>
                </label>
              ...
              <div>
                <label className="label">
                  Poll Interval
                  <span className="text-xs text-gray-400 font-normal ml-1">(seconds)</span>
                </label>
          ...
          <div className="rounded-lg border border-teal-200 dark:border-teal-800 overflow-hidden">
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-teal-800 dark:text-teal-300">
                  How to enable SNMP on: {guide.title}
                </span>
              </div>
            ...
            {guideOpen && (
              <div className="p-4 bg-white dark:bg-gray-900 border-t border-teal-100 dark:border-teal-800">
                <div className="bg-teal-50 dark:bg-teal-900/20 rounded p-3 mb-3 text-xs text-teal-700 dark:text-teal-300 grid grid-cols-2 gap-2">
                  <span><strong>Port:</strong> UDP 161</span>
                  <span><strong>Protocol:</strong> {form.snmp_version.toUpperCase()}</span>
                  <span><strong>Community:</strong> {form.snmp_community || 'public'}</span>
                  <span><strong>Direction:</strong> NMS polls device</span>
                </div>
                <ol className="space-y-2">
                  {guide.steps.map((step, i) => (
                    <li key={i} className="flex gap-2.5 text-xs text-gray-700 dark:text-gray-300">
                      <span className="w-5 h-5 rounded-full bg-teal-100 dark:bg-teal-900 text-teal-700 dark:text-teal-300 flex items-center justify-center flex-shrink-0 font-bold text-xs mt-0.5">
                        {i + 1}
                      </span>
...
export default function Devices() {
  ...
  return (
    <div className="flex gap-6 h-full animate-fade-in">
      {showAdd && (
        <AddDeviceModal onClose={() => setShowAdd(false)} onSaved={onDeviceSaved} />
      )}
            ...
            {filtered.length === 0 && (
              <div className="p-12 text-center text-gray-400">
                ...
                <p className="text-sm mt-1">
                  Use <button onClick={() => setShowAdd(true)} className="text-teal-600 underline">Add Device</button> to add manually, or go to <strong>Auto-Discover</strong> to scan your network.
                </p>
              </div>
            )}
          </div>
